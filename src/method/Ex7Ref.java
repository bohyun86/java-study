    ...
    public static void main(String[] args) {
        ...
        while (tries < MAX_TRIES) {
            ...
            for (int j = 0; j < selectedWord.length(); j++) {
                if (guessLetter == selectedWord.charAt(j)) {
                    isCorrectGuess = true;
                    displayArray[j] = selectedWord.charAt(j);
                }
            }

            if (!isCorrectGuess) {
                tries += 1;
                System.out.println(guessLetter + "는 없는 글자입니다. 남은 시도: " + (MAX_TRIES - tries) );
                continue;
            }

            // 게임 종료 조건 검사 및 게임 결과 출력
            if (isCorrectGuess) {
                for (char k:displayArray) {
                    if (k == '_') {
            ...
            if (isCorrectGuess) {
                System.out.println("축하합니다. " + tries + "정답(" + selectedWord + ")을 맞추셨습니다.");
                break;
            }
        }
    }
